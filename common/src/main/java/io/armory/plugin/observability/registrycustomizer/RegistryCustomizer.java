...
public abstract class RegistryCustomizer implements MeterRegistryCustomizer<MeterRegistry> {
  ...
  public void customize(MeterRegistry registry) {
    var registryClass = registry.getClass().getSimpleName();
    try {
      ...
      registryClass = registryImpl.getClass().getSimpleName();
    } catch (NoSuchFieldException e) {
      // DO NOTHING
    }
    log.info(
        "{} is preparing to customize {} registry class if enabled",
        this.getClass().getSimpleName(),
        registryClass);

    doCustomize(registry);
  }
